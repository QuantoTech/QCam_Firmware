...
static int set_contrast(sensor_t *sensor, int level)
{
    ...
    WRITE_REG_OR_RETURN(BANK_DSP, 0xFF, 0x00); //added for qtech
    for (int i=0; i<7; i++) {
        WRITE_REG_OR_RETURN(BANK_DSP, contrast_regs[0][i], contrast_regs[level][i]);
    }
    WRITE_REG_OR_RETURN(BANK_DSP, 0xFF, 0x00);//added for qtech
    WRITE_REG_OR_RETURN(BANK_DSP, 0x7c, 0x00);//added for qtech
    WRITE_REG_OR_RETURN(BANK_DSP, 0x7d, 0x07);//added for qtech
    return ret;
}
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    WRITE_REG_OR_RETURN(BANK_DSP, 0xFF, 0x00);
    WRITE_REG_OR_RETURN(BANK_DSP, 0x7c, 0x00);
    WRITE_REG_OR_RETURN(BANK_DSP, 0x7d, 0x07);
    return ret;
}
